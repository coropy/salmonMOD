...
public record MultiSurfacePaintResult(
     ...
     * @param face         面方向（後方互換用・描画には {@link #surfaceKey()} を使用）
     * @param surfaceKey   完全なサーフェスキー（Patch ID を含む）
    ...
    public record UpdatedInkSurface(
        ...
        public static UpdatedInkSurface from(BlockPos blockPos, Direction face,
            ...
            return new UpdatedInkSurface(blockPos, face, surfaceKey,
                    faceData.copyCells(), changedCells);
        }
    }
}
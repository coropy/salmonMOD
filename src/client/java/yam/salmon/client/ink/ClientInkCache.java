 ...
 *
 * <p>Phase 8: キーに InkSurfacePatchId を含め、部分ブロックの
 * 複数パッチ（階段の下段踏面・上段踏面など）を別々に保持可能。</p>
 */
public class ClientInkCache {
    ...
    public void addSyncFace(
            ...
            BlockPos blockPos, Direction face, InkSurfacePatchId patchId,
            byte[] cells, long revision) {
        if (!syncInProgress) {
            // 完全同期中でなければ単独で適用
            applyFaceUpdate(arenaUuid, arenaNumber, dimensionId,
                    blockPos, face, patchId, cells, revision);
            return;
        }
        ...
        ClientInkSurface surface = createSurface(arenaUuid, arenaNumber, dimensionId,
                blockPos, face, patchId, cells, revision);
        if (surface != null) {
    ...
    public void applyFaceUpdate(
        ...
        if (patchId == null) {
            Salmon.LOGGER.warn("[Client] Invalid patchId in update");
            return;
        }

        // 完全同期中ならバッファへ
        if (syncInProgress) {
            addSyncFace(arenaUuid, arenaNumber, dimensionId,
                    blockPos, patchId.normal(), patchId, cells, revision);
            return;
        }
        ...
        if (dimCache != null) {
            ...
            if (arenaMap != null) {
                ClientInkSurfaceKey key = new ClientInkSurfaceKey(arenaUuid, blockPos, patchId);
                ClientInkSurface existing = arenaMap.get(key);
                if (existing != null && existing.revision() > revision) {
                    // 既存のほうが新しい → この更新を無視
                    Salmon.LOGGER.debug("[Client] Old revision ignored: arena={} pos={} patch={} existingRev={} newRev={}",
                            arenaUuid, blockPos, patchId, existing.revision(), revision);
                    return;
                }
            }
        }
        ...
        if (!existed) {
            Salmon.LOGGER.debug("[Client] Cache surface created: arena #{} pos={} patch={}",
                    arenaNumber, blockPos, patchId);
        } else {
            Salmon.LOGGER.debug("[Client] Cache surface updated: arena #{} pos={} patch={}",
                    arenaNumber, blockPos, patchId);
        }
    }
    ...
    private ClientInkSurface createSurface(
        ...
        return new ClientInkSurface(
                ...
                patchId, normalizedCells, revision, teamACount, teamBCount);
    }
